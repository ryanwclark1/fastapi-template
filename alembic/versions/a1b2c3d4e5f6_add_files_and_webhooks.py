...
def upgrade() -> None:
    ...
    file_status_enum = postgresql.ENUM(
        "pending", "processing", "ready", "failed", "deleted", name="filestatus", create_type=True
    )
    file_status_enum.create(op.get_bind(), checkfirst=True)

    # Create delivery status enum
    delivery_status_enum = postgresql.ENUM(
        "pending", "delivered", "failed", "retrying", name="deliverystatus", create_type=True
    )
    ...
    op.create_index(
        "ix_webhooks_event_types", "webhooks", ["event_types"], unique=False, postgresql_using="gin"
    )
    ...
    op.create_index(
        "ix_webhook_deliveries_event_type", "webhook_deliveries", ["event_type"], unique=False
    )
    op.create_index(
        "ix_webhook_deliveries_next_retry_at", "webhook_deliveries", ["next_retry_at"], unique=False
    )
    op.create_index(
